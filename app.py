...
def build_cypher(df: pd.DataFrame, source_col: str, relation_col: str, target_col: str) -> str:
    def _cypher_escape(s: object) -> str:
        """Escape a Python string for safe inclusion in a Cypher single-quoted literal.

        Uses Cypher escaping convention: single quotes are doubled. Also strips
        newlines and trims whitespace to avoid multi-line values in node names.
        """
        if s is None:
            return ""
        stxt = str(s)
        ...
        stxt = re.sub(r"[\r\n]+", " ", stxt).strip()
        # Double single quotes for Cypher escaping
        return stxt.replace("'", "''")

    header = [
        "// Validated Knowledge Graph — Generated by QA Validation Tool",
        ""
    ]
...
""")
    ...
    # ── Cypher Export ─────────────────────────────────────────────────────────
    st.markdown("---")
    st.subheader("📄 Full Neo4j Cypher Export")
    st.info(
        "Copy the entire script below or download the .cypher file."
    )
    ...
    st.download_button(
        ...
        data=cypher_text,
        file_name="qa_validation_tool_knowledge_graph.cypher",
        mime="text/plain",
    )
